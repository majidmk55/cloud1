...
const criteria = [
  ...
  { id: 9, text: 'Database schema شامل Partner, RevenueSplitContract, FinancialLedger, PartnerSettlement', cat: 'Database' },
...
export function DoD() {
  return (
    <div className="space-y-8">
      ...
      <div className="bg-gradient-to-br from-emerald-500/10 to-transparent rounded-2xl border border-emerald-500/30 p-6">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-lg font-bold text-white">پیشرفت فاز صفر</h2>
          <span className="text-3xl font-black text-emerald-400">{criteria.length}/{criteria.length}</span>
        </div>
      ...
      {categories.map((cat) => (
        <div key={cat} className="bg-gradient-to-br from-white/5 to-transparent rounded-2xl border border-white/10 p-5">
          ...
          <div className="space-y-2">
            {criteria.filter(c => c.cat === cat).map((c) => (
              <div key={c.id} className="flex items-start gap-3 bg-[#050816] rounded-xl p-3 border border-white/5">
      ...
      <div className="bg-gradient-to-br from-white/5 to-transparent rounded-2xl border border-white/10 p-6">
        <h2 className="text-lg font-bold text-white mb-4">🚀 راهنمای شروع سریع</h2>
        <div className="space-y-3">
          {[
            ...
            <div key={step.title} className="bg-[#050816] rounded-xl p-4 border border-white/5">
              <h3 className="text-sm font-bold text-blue-400 mb-2">{step.title}</h3>
      ...
      <div className="bg-gradient-to-br from-white/5 to-transparent rounded-2xl border border-white/10 p-6">
        <h2 className="text-lg font-bold text-white mb-4">🗺️ نقشه راه فازها</h2>
        <div className="space-y-2">
          {[
            ...
            { phase: 'Phase 3', name: 'Provisioning & Lifecycle', status: 'planned', desc: 'استقرار، Provider integration' },
            { phase: 'Phase 4', name: 'Reseller System', status: 'future', desc: 'پورتال ریسلر، white-label' },
            { phase: 'Phase 5', name: 'Inventory & Config', status: 'future', desc: 'مدیریت منابع، capacity planning' },
            { phase: 'Phase 6-10', name: 'Enhancement', status: 'future', desc: 'GPU/AI، networking، backup' },
            { phase: 'Phase 11', name: 'Analytics & BI', status: 'future', desc: 'داشبوردها، گزارش‌ها، KPIs' },
          ].map((p) => (
            <div key={p.phase} className="flex items-center gap-3 bg-[#050816] rounded-xl p-3 border border-white/5">
